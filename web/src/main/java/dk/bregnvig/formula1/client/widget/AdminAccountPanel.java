	...
	public AdminAccountPanel(F2007 mediator, MainPanel mainPanel) {
		...
		players.addClickListener(new ClickListener() {
			public void onClick(Widget arg0) {
				...
				long id  = Long.parseLong(players.getValue(players.getSelectedIndex()));
	...
	private void loadDrivers() {
		...
		AsyncCallback callback = new AsyncCallback() {
			...
			public void onSuccess(Object argument) {
				...
				int previousSelection = players.getSelectedIndex();
				previousSelection = previousSelection == -1 ? 0 : previousSelection;
				players.clear();
				players.addItem("Opret");
				for (int i = 0; i < fetchedDrivers.size(); i++) {
					ClientDriver driver = (ClientDriver) fetchedDrivers.get(i);
					players.addItem(driver.getName(), driver.getId().toString());
				}
				players.setSelectedIndex(previousSelection);
			}
		};
